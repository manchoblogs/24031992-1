...
function addcomment(){
	...
	var comment=$('textarea[name=comment_text]').val(),spoiler=$('input[name=spoiler]:checked').val(),contenttype=POSTurl,id=POSTid;
	...
	$.post(request_url,{'id':id,'contenttype':contenttype,'comment':comment,'spoiler':spoiler,'user_username':user_username,'user_email':user_email,'user_password':user_password,'AccessToken':AccessToken,'CUSER':CUSER,'CUSER_NAME':CUSER_NAME,'CUSER_LINK':CUSER_LINK,'CUSER_ID':CUSER_ID, 'CUSER_ICON':CUSER_ICON,'type':'addcomment'},
	function(response){
...
function comment_like(id,contenttype){$.post(request_url,{'id':id,'tip':'like','contenttype':contenttype,'CUSER':CUSER,'CUSER_ID':CUSER_ID, 'AccessToken':AccessToken,'type':'comment_like_func'},function(response){if(response.error){error(response.error);}else{$('#like_'+ id).text(parseInt($('#like_'+ id).text())+ 1);}},'json');} 
function comment_dislike(id,contenttype){$.post(request_url,{'id':id,'tip':'unlike','contenttype':contenttype,'CUSER':CUSER,'CUSER_ID':CUSER_ID, 'AccessToken':AccessToken,'type':'comment_like_func'},function(response){if(response.error){error(response.error);}else{$('#dislike_'+ id).text(parseInt($('#dislike_'+ id).text())+ 1);}},'json');}
...
function CommentDelete(id){if(confirm(lang_3)) {$.post(request_url,{'id':id,'type':'CommentDelete'},function(response){ if(response.error){error(response.error);}else{$("#comment"+id).remove();}},'json');}}
