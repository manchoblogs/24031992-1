		...
		if (isset($_GET['uploadfile'])){ 
			...
			if (isset($_FILES['avatar']['tmp_name'])){
				require_once("../comments/inc/upload/uploadfunction.php"); 
				 ...
				 $resimdonen=resimgo($_FILES['avatar']['name'], $_FILES['avatar']['type'], $_FILES['avatar']['error'], $_FILES['avatar']['tmp_name'] , 'member/avatar', "$_SESSION[seoslug]","$olanresim","","");
				...
				ECHO '<div class="alert"><button type="button" class="close" data-dismiss="alert">×</button> <strong>Alert!</strong> Must be: Gif,Png,Jpg.</div>';
					...
					$_SESSION['icon']=$resimdonen;
				 	$return2 = $dbpdo->prepare("UpDate users Set  icon = '$resimdonen' where id = '$_SESSION[oturumid]'");
