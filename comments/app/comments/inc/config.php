<?php
	...
	$user_id	=  $_COOKIE[$db_sitemotto."user"];
	$password	=  $_COOKIE[$db_sitemotto."pass"];
	

		if(!isset($_SESSION['last_date']) or (time() - $_SESSION['last_date']) > 60 ){
		$rsqa  = $dbpdo->query("Select id,user_id,password,usertype,seoslug,icon,ban,last_date from users where user_id = '$user_id' and password = '$password'");
			...
			setTimeout(function(){ location.href = 'admin/logout.php'; }, 2000);
			});</script>");
				...
				$return3 = $dbpdo->prepare("UpDate users Set ipno = '$ip' , last_date = '$phptarih' where id = '$gelenz[id]'");
