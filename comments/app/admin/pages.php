...
if(isset($_GET['page'])) {
	...
	if($page=="about"){$pagename= 'About Page';
		...
		$rsqaj = $dbpdo->query("select * from pages where content_type = '$page'");
	
		if ($yeni = $rsqaj->fetch()) {
		$id=$yeni["id"];
		$text=$yeni["text"];
		$date=$yeni["date"];
		$content_type=$yeni["content_type"];
		}else{
		$id="";
		$text="";
		$date="";
					...
					<div class="widget-content">
						...
						if (isset($_GET['success'])){ 
							...
							$text = $_POST["text"];
								...
								$rsqaj = $dbpdo->query("select * from pages where content_type = '$pagetip'")->rowCount();
								
								
								if ($rsqaj=="") {
								
								 $dbpdo->exec("INSERT INTO pages (text,content_type,date) VALUES ('$text' , '$pagetip' , '$phpdate')");

								header("Location: ?page=$page&success=Created.");exit();
								}else{
									$dbpdo->exec("UpDate pages Set text = '$text', date = '$phpdate' where content_type = '$pagetip'");
						...
						<input name="pagetip" id="pagetip" type="hidden" value="<?php echo $page; ?>">
							<fieldset>
										...
										<div class="control-group " style="width:100%">
												<b>Last Update:</b> <?php if($date==""){echo "This page not created.";}else{echo timeConvert($date); }?>
										</div>
										...
										<div class="control-group">											
											...
											<div class="controls">
												...
												<textarea name="text" id="text" style="width:100%;height:400px"><?php echo $text ?></textarea>
