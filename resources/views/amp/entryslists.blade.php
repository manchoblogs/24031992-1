@foreach($entrys as $key => $entry)
    ...
    @if($entry->type=='image')
    <p><amp-img alt src="{{ makepreview($entry->image, null, 'entries') }}" width="840" height="472" layout="responsive"></amp-img></p>
    @endif
    ...
    @if( $entry->type=='video')
        ...
        <?php
        $my_ar = explode("facebook.com", $entry->video);
        if(isset($my_ar[0])){  ?>
        <p><amp-facebook data-videoid="{{ $entry->video }}" layout="responsive" data-embed-as="video" width="480" height="270"></amp-facebook></p>
        <?php
        } ?>
