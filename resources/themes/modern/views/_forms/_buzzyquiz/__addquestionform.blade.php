...
<div class="entry" data-type="quizquestion" data-co="{{ $uniquid }}" @if(isset($entry->id)) data-entry-id="{{ $entry->id }}" @endif>
    ...
    <div class="moredetail questionmore">
        ...
        <div class="clearfix"></div>
        <a class="button button-white button-full submit-button postable answerbutton" data-method="Get" data-target="answer{{ $uniquid }}" data-puttype="append" data-type="answerform" href="{{ action('FormController@addnewform') }}?addnew=answer{{ Request::query('qtype')=='trivia' ? '&qtype=trivia' : '' }}"><i class="fa fa-plus-circle"></i>{{ trans('addpost.add', ['type' => trans('buzzyquiz.answer')]) }}</a>
    </div>
</div>
