...
<div class="entry" data-type="poll" data-co="{{ $uniquid }}" @if(isset($entry->id)) data-entry-id="{{ $entry->id }}" @endif>
    ...
    <div class="moredetail questionmore">
        ...
        <div class="clearfix"></div>
        <a class="button button-white button-full submit-button postable answerbutton clearfix" data-method="Get" data-target="answer{{ $uniquid }}" data-puttype="append" data-type="pollanswerform" href="{{ action('FormController@addnewform') }}?addnew=pollanswer"><i class="fa fa-plus-circle"></i>{{ trans('addpost.add', ['type' => trans('buzzyquiz.answer')]) }}</a>
    </div>
</div>
